...
import foloSend from '../lib/foloSend'
...
export default function RecipientsInput(props) {
  ...
  const filterRecordsOnChain = async (token, records) => {
    ...
    let unpreparedAddresses = await foloSend.batchQueryReceiver(token, addresses)
  ...
  const filterRecords = (rawRecordsStr) => {
    ...
    for (var i = 0; i < rawRecords.length; i++) {
      ...
      try {
        ...
        // check for email, twitter handle, discord handle
        console.log(address)
        if(address.includes(".") && address.includes("@")) {
          if(! address.match(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/) ) throw "invalid address"
        } else if(address.startsWith("@")) { // twitter handle
          if(!address.substring(1).match( /^[A-Za-z0-9_]{1,15}$/ )) throw "invalid address"
        } else if(address.includes("#")) { // discord handle
          if(!address.match(/^((.{2,32})#\d{4}$)/)) throw "invalid address"
        } else {
    ...
    setRecordsSum(records.reduce((p, c) => {
      return p.add(c.amount)
    }, new Decimal(0)))
    ...
    <>
      <div className={props.className}>
        ...
        <label className="block font-flow text-md leading-7">
          For each line, enter one address (email, twitter, discord) and the token amount, seperate with comma.
        </label>
        <div className="mt-1">
          <textarea
            rows={4}
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (props.selectedToken && rawRecordsStr.trim().length > 0) {
                ...
                const [prepared, unprepared] = [records, []] 
            ...
            <>
              ...
              <div className="flex gap-x-4 mt-8 mb-20 items-end h-14 mb-30">
                <button
                  ...
                  onClick={async () => {
                    cleanTxInfo()
                    console.log("selectedToken", props.selectedToken)
                    if (props.selectedToken) {
                      try {
                        //const transactionId = await foloSend.batchTransfer(props.selectedToken, validRecords)
                        const transactionId = await foloSend.foloSendBatch(props.selectedToken, validRecords)
